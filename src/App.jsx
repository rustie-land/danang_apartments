...
export default function App() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      if (data) {
        setApartments(data);
        const wordFreq = {};
        // Тот самый белый список слов для недвижимости
  ...
  return (
    ...
    >
      {/* КНОПКА-ПОДСКАЗКА, ЕСЛИ ЗАКРЫТО */}
      {!isOpen && (
        <div style={styles.openHint} onMouseDown={handleStart} onTouchStart={handleStart}>
          <span>Swipe to search →</span>
        </div>
      )}

      {/* САЙДБАР С ВЕРНУВШИМСЯ ДИЗАЙНОМ */}
      <div 
        style={{ 
          ...styles.sidebar, 
      ...
      >
        <div style={styles.sidebarHeader} onMouseDown={handleStart} onTouchStart={handleStart}>
          ...
          <div style={styles.filterBox}>
            ...
            <input 
              type="text" 
              placeholder="Search features (pool, sea...)" 
              value={tagSearch}
      ...
      <div style={styles.mapWrapper}>
        <MapContainer center={[16.0544, 108.2422]} zoom={13} style={{ height: '100%', width: '100%' }}>
          ...
          {filteredApts.map(apt => (
            <Marker key={apt.id} position={[apt.lat, apt.lng]} icon={hoveredId === apt.id ? activeIcon : defaultIcon} />
          ))}
        </MapContainer>
      </div>
...
const styles = {
  ...
  openHint: { position: 'absolute', left: 0, top: '50%', transform: 'translateY(-50%)', zIndex: 1001, background: '#1e293b', color: '#fff', padding: '15px 10px', borderRadius: '0 15px 15px 0', cursor: 'e-resize', writingMode: 'vertical-rl', fontSize: '12px', fontWeight: 'bold', boxShadow: '5px 0 15px rgba(0,0,0,0.2)' },
  sidebar: { 
    position: 'absolute', left: 0, top: 0, width: '420px', height: '100vh', overflowY: 'auto', 
    background: '#fff', zIndex: 1000, boxShadow: '10px 0 40px rgba(0,0,0,0.1)', userSelect: 'none' 
  },
  sidebarHeader: { padding: '20px 24px', cursor: 'grab' },
  ...
  select: { padding: '14px', borderRadius: '14px', border: '1px solid #e2e8f0', background: '#fff', fontSize: '14px' },
  list: { padding: '0 24px 40px 24px' },
  card: { background: '#fff', borderRadius: '28px', overflow: 'hidden', cursor: 'pointer', marginBottom: '25px', border: '3.5px solid transparent', boxShadow: '0 10px 20px rgba(0,0,0,0.05)', transition: '0.3s' },
  ...
  modalPrice: { fontSize: '36px', fontWeight: '900', marginBottom: '15px' },
