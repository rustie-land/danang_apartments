...
const highlightIcon = createIcon('black', true);
      ...
      <div style={styles.mapWrapper}>
        <MapContainer center={[16.0544, 108.2422]} zoom={13} zoomControl={false} style={{ height: '100%', width: '100%' }}>
          ...
          {filteredApts.map(apt => (
            <Marker key={apt.id} position={[apt.lat, apt.lng]} icon={hoveredAptId === apt.id ? highlightIcon : defaultIcon} eventHandlers={{ click: () => setSelectedApt(apt) }} />
          ))}
        </MapContainer>
      </div>
          ...
          <div className="sidebar-scroll-container">
            <div style={styles.sidebarHeader}>
              ...
              <div className="range-slider-container">
                ...
                <input type="range" className="dual-range" min={priceRange.min} max={priceRange.max} step={500000} value={currentFilter.min} onChange={(e) => setCurrentFilter(prev => ({ ...prev, min: Math.min(Number(e.target.value), prev.max - 1000000) }))} />
                <input type="range" className="dual-range" min={priceRange.min} max={priceRange.max} step={500000} value={currentFilter.max} onChange={(e) => setCurrentFilter(prev => ({ ...prev, max: Math.max(Number(e.target.value), prev.min + 1000000) }))} />
              </div>
              <div style={styles.tagGrid}>
                {['studio', '1br', '2br', '3plus'].map(t => (
                  <button key={t} onClick={() => setPropertyType(propertyType === t ? 'all' : t)} style={{ ...styles.typeChip, backgroundColor: propertyType === t ? '#1d1d1f' : '#f5f5f7', color: propertyType === t ? '#fff' : '#1d1d1f' }}>{t.toUpperCase()}</button>
                ))}
              </div>
              <div style={styles.hashtagsRow}>
                {dynamicTags.map(tag => (
                  <button key={tag} onClick={() => setSelectedTags(prev => prev.includes(tag) ? prev.filter(t => t !== tag) : [...prev, tag])} style={{ ...styles.tag, backgroundColor: selectedTags.includes(tag) ? '#007AFF' : '#fff', color: selectedTags.includes(tag) ? '#fff' : '#86868b' }}>#{tag}</button>
                ))}
              </div>
            </div>
            ...
            <div style={styles.list}>
              {filteredApts.map(apt => (
                <div key={apt.id} className="apt-card" onClick={() => setSelectedApt(apt)} onMouseEnter={() => setHoveredAptId(apt.id)} onMouseLeave={() => setHoveredAptId(null)}>
