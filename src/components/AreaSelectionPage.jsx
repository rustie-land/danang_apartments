...
export default function AreaSelectionPage({ initialCenter, initialZoom, onBoundsChange }) {
  const { t } = useLang();
  const { properties, filterByPreferences } = useFilters();
  const navigate = useNavigate();
  const [bounds, setBounds] = useState(null);

  const handleBounds = (b) => {
    setBounds(b);
    onBoundsChange?.(b);
  };

  const inBounds = properties.filter(filterByPreferences).filter((item) => {
    if (!bounds) return true;
    return bounds.contains([item.lat, item.lng]);
  });

  const done = () => navigate('/results');

  return (
    <div style={{ height: 'calc(100vh - 56px)', width: '100%', position: 'relative', fontFamily: 'var(--font-sans)' }}>
      <div style={{ position: 'absolute', top: '20px', left: '20px', right: '20px', zIndex: 1000, display: 'flex', justifyContent: 'space-between', alignItems: 'center', pointerEvents: 'none', flexWrap: 'wrap', gap: '0.75rem' }}>
        <div style={{ pointerEvents: 'auto', backgroundColor: 'var(--color-deep)', color: 'var(--color-bg)', padding: '0.6rem 1.2rem', borderRadius: '2rem', boxShadow: 'var(--shadow-soft)', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          <span style={{ fontSize: '0.8rem', fontWeight: 600 }}>{t('selectAreaTitle')}</span>
        </div>
      ...
      <MapContainer center={initialCenter} zoom={initialZoom} style={{ height: '100%', width: '100%' }}>
        ...
        <AreaListener onBoundsChange={handleBounds} />
        {inBounds.map((prop) => (
      ...
      >
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontFamily: 'var(--font-serif)', fontSize: '1.4rem', fontWeight: 600, marginTop: '0.2rem' }}>
            {inBounds.length === 0 ? t('noResults') : `${inBounds.length} ${t('objectsInZone')}`}
          </div>
          <div style={{ fontSize: '0.75rem', color: 'rgba(255,255,255,0.6)', marginTop: '0.1rem' }}>
            {inBounds.length === 0 ? t('selectAreaHint') : t('changeZone')}
          </div>
        </div>
        ...
        <button
          ...
          style={{
            backgroundColor: inBounds.length > 0 ? 'var(--color-accent)' : 'var(--color-disabled)',
            color: '#fff',
            border: 'none',
            ...
            fontWeight: 700,
            fontSize: '0.85rem',
            cursor: inBounds.length > 0 ? 'pointer' : 'not-allowed',
            whiteSpace: 'nowrap',
