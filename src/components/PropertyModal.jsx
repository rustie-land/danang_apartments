    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          style={{ backgroundColor: 'var(--as-surface)', borderRadius: 'var(--as-radius-xl)', maxWidth: '600px', width: '100%', maxHeight: '90vh', overflowY: 'auto', boxShadow: 'var(--as-shadow-strong)', position: 'relative' }}
        >
          <button
            ref={closeButtonRef}
            onClick={onClose}
          ...
          <div style={{ padding: '1.75rem 1.75rem 2rem' }}>
            <div style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--as-accent)', letterSpacing: '0.05em' }}>
              {property.area && property.area !== 'L' ? property.area.toUpperCase() : 'DA NANG'}
            </div>
            <h2 style={{ fontFamily: 'var(--as-font-serif)', fontSize: '2rem', color: 'var(--as-text)', margin: '0.25rem 0 0.5rem 0' }}>
              {property.title}
            </h2>
            <div style={{ fontSize: '0.85rem', color: 'var(--as-text-muted)', marginBottom: '1rem' }}>📍 {property.address && property.address !== 'L' ? property.address : (property.area && property.area !== 'L' ? property.area : 'Da Nang')}</div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px,1fr))', gap: '0.75rem', marginBottom: '1.25rem' }}>
              <div style={{ backgroundColor: 'var(--as-surface-alt)', border: '1px solid var(--as-border)', borderRadius: 'var(--as-radius-card)', padding: '0.75rem' }}>
            ...
            <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1.5rem', flexWrap: 'wrap' }}>
              {property.amenities.map((a) => (
                <span key={a} style={{ backgroundColor: 'var(--as-surface-alt)', color: 'var(--as-text)', padding: '0.3rem 0.75rem', borderRadius: '1rem', fontSize: '0.75rem', fontWeight: 600, border: '1px solid var(--as-border)' }}>
                  {a}
                </span>
              ))}
            </div>
            ...
            {/* Host profile */}
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', backgroundColor: 'var(--as-surface-alt)', border: '1px solid var(--as-border)', borderRadius: 'var(--as-radius-card)', padding: '0.85rem', marginBottom: '1.5rem' }}>
              <div style={{ width: '44px', height: '44px', borderRadius: '50%', background: 'linear-gradient(135deg,#0D9488,#F97316)', flexShrink: 0 }}></div>
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 700, color: 'var(--as-text)' }}>{property.host || 'Direct owner'} <span style={{ fontSize: '0.7rem', color: 'var(--as-accent)', fontWeight: 600 }}>✓ Direct</span></div>
